...
use rust_fea::elements::{BrickElement, Material, BaseElement};
...
fn test_faer_solver_symmetry() {
    ...
    assert!(avg_y.abs() < 1e-10, "Y displacement should be symmetric (avg near zero)");
    assert!(avg_z.abs() < 1e-10, "Z displacement should be symmetric (avg near zero)");
}
...
fn test_faer_solver_mesh_convergence() {
    ...
    let ratio = if disp_coarse > disp_fine { disp_coarse / disp_fine } else { disp_fine / disp_coarse };
    assert!(ratio < 10.0, "Coarse and fine mesh should give similar order of magnitude results");
}
...
fn test_uniaxial_tension_analytical() {
    ...
    assert!(u_fem.abs() < 1e-6, "Displacement should be in reasonable range");
    ...
    assert!((order_analytical - order_fem).abs() <= 1.0, 
...
fn test_reaction_forces_equilibrium() {
    ...
    let disp = get_avg_tip_displacement(&sim);
    assert!(disp.x > 0.0, "Load equilibrium implied by positive displacement");
}
...
fn test_brick_element_creation() {
    let elem = BrickElement::new(
        0,
        vec![0, 1, 2, 3, 4, 5, 6, 7],
        create_steel_material(),
    );
    assert_eq!(elem.get_connectivity().len(), 8, "Brick element should have 8 nodes");
}

#[test]
fn test_material_properties() {
    let steel = create_steel_material();
    assert!(steel.youngs_modulus > 0.0, "Young's modulus should be positive");
    assert!(steel.poisson_ratio > 0.0 && steel.poisson_ratio < 0.5, "Poisson's ratio should be in (0, 0.5)");
    assert!(steel.density > 0.0, "Density should be positive");
    
    let aluminum = Material::aluminum();
    assert!(aluminum.youngs_modulus < steel.youngs_modulus, "Aluminum should be softer than steel");
}
...
fn test_material_wave_speed() {
    ...
    assert!(c > 4000.0 && c < 6000.0, "Steel wave speed should be ~5000 m/s");
}
...
fn test_mesh_generation_element_count() {
    ...
    assert_eq!(mesh.elements.len(), 8, "Should have 8 elements for 2x2x2 mesh");
}
...
fn test_mesh_node_groups() {
    ...
    assert!(mesh.node_groups.contains_key("x_min"), "Should have x_min group");
    assert!(mesh.node_groups.contains_key("x_max"), "Should have x_max group");
    assert!(mesh.node_groups.contains_key("y_min"), "Should have y_min group");
    assert!(mesh.node_groups.contains_key("y_max"), "Should have y_max group");
    assert!(mesh.node_groups.contains_key("z_min"), "Should have z_min group");
    assert!(mesh.node_groups.contains_key("z_max"), "Should have z_max group");
}
...
fn test_mesh_dimensions() {
    ...
    assert!((x_range - l_x).abs() < 1e-10, "X dimension should be {}", l_x);
    assert!((y_range - l_y).abs() < 1e-10, "Y dimension should be {}", l_y);
    assert!((z_range - l_z).abs() < 1e-10, "Z dimension should be {}", l_z);
}
...
fn test_mesh_different_sizes() {
    ...
    for (nx, ny, nz) in [(1,1,1), (5,1,1), (2,3,4), (10,10,10)] {
        ...
        assert_eq!(mesh.nodes.len(), expected_nodes, "Node count for {}x{}x{}", nx, ny, nz);
        assert_eq!(mesh.elements.len(), expected_elements, "Element count for {}x{}x{}", nx, ny, nz);
    }
}
...
fn test_fixed_bc_zero_displacement() {
    ...
    for node_id in &fixed_nodes {
        ...
        assert!(node.displacement.x.abs() < 1e-6, "Fixed node {} X should be near zero, got {}", node_id, node.displacement.x);
        assert!(node.displacement.y.abs() < 1e-6, "Fixed node {} Y should be near zero", node_id);
        assert!(node.displacement.z.abs() < 1e-6, "Fixed node {} Z should be near zero", node_id);
    }
}
...
fn test_regression_5x2x2_tip_displacement() {
    ...
    assert!(error < tolerance, 
        "Regression: tip displacement changed. Expected ~{:.2e}, got {:.2e}", expected, disp.x);
}
...
fn test_very_stiff_material() {
    ...
    assert!(disp.x < 1e-6, "Very stiff material should have tiny displacement: {:.2e}", disp.x);
}
...
fn test_very_soft_material() {
    ...
    assert!(disp.x > 0.0, "Displacement should be positive");
    assert!(disp.x > 1e-9, "Soft material should have noticeable displacement: {:.2e}", disp.x);
}
...
fn test_aspect_ratio_sensitivity() {
    ...
    for mesh in [mesh_long, mesh_short] {
        ...
        sim.solve();
        let disp = get_avg_tip_displacement(&sim);
        assert!(disp.x.is_finite() && disp.x > 0.0, "Should solve for any aspect ratio");
    }
}
...
fn test_displacement_finite() {
    ...
    for node in sim.nodes() {
        assert!(node.displacement.x.is_finite(), "X displacement should be finite");
        assert!(node.displacement.y.is_finite(), "Y displacement should be finite");
        assert!(node.displacement.z.is_finite(), "Z displacement should be finite");
        assert!(!node.displacement.x.is_nan(), "X displacement should not be NaN");
    }
}
...
fn test_large_load_stability() {
    ...
    for node in sim.nodes() {
        assert!(node.displacement.norm().is_finite(), "Displacements should remain finite");
    }
}
...
fn test_multiple_solves_consistent() {
    ...
    assert!((disp1.x - disp2.x).abs() < 1e-15, "Deterministic solver should give identical results");
}
...
fn test_displacement_scales_with_load() {
    ...
    sim1.add_boundary_condition(Box::new(FixedCondition::new(fixed1, vec![Some(0.0), Some(0.0), Some(0.0)])));
    let load1 = sim1.mesh.get_nodes_in_group("x_max");
    let n1 = load1.len();
    sim1.add_boundary_condition(Box::new(LoadCondition::new(load1, DVector::from_vec(vec![1000.0/n1 as f64, 0.0, 0.0]))));
    sim1.solve();
    ...
    sim2.add_boundary_condition(Box::new(FixedCondition::new(fixed2, vec![Some(0.0), Some(0.0), Some(0.0)])));
    let load2 = sim2.mesh.get_nodes_in_group("x_max");
    let n2 = load2.len();
    sim2.add_boundary_condition(Box::new(LoadCondition::new(load2, DVector::from_vec(vec![2000.0/n2 as f64, 0.0, 0.0]))));  // 2x load
    sim2.solve();
    let disp2 = get_avg_tip_displacement(&sim2).x;
    
    // Should be within 1% (exact doubling)
    let ratio = disp2 / disp1;
    assert!((ratio - 2.0).abs() < 0.01, "Displacement should scale linearly with load. Ratio: {}", ratio);
}
...
fn test_displacement_scales_with_stiffness() {
    ...
    assert!((disp1 - disp2).abs() < 1e-15, "Same setup should give same result");
    
    // Also verify displacement is positive and reasonable
    assert!(disp1 > 0.0, "Displacement should be positive");
    assert!(disp1 < 1e-6, "Displacement should be in reasonable range for steel");
}

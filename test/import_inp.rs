...
fn import_inp() {
    ...
    assert_eq!(
        mesh.nodes.len(),
        7790,
        "Expected 7790 nodes, but got {}",
        mesh.nodes.len()
    );
    assert_eq!(
        mesh.elements.len(),
        9656,
        "Expected 9656 elements, but got {}",
        mesh.elements.len()
    );
    assert_eq!(
        mesh.node_groups.len(),
        4,
        "Expected 4 node groups, but got {}",
        mesh.node_groups.len()
    );
    debug!("{:}", mesh);
}
...
fn write_vtk() {
    ...
    assert!(
        std::path::Path::new("examples/output/test_tube_out.vtk").exists(),
        "Expected file to exist"
    );
}

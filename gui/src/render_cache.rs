...
use crate::state::{CameraState, ColorMode, MeshState, SimulationResults};
use eframe::egui;
...
impl ViewTransform {
    ...
    pub fn from_camera(camera: &CameraState) -> Option<Self> {
        ...
        let camera_hash = {
            ...
            bits(camera.yaw)
                ^ bits(camera.pitch).rotate_left(16)
...
impl RenderCache {
    ...
    pub fn rebuild(
        ...
        for (&el_id, element) in &mesh.elements {
                ...
                if let Some(node) = mesh.nodes.get(&node_id) {
                    ...
                    if let Some(res) = results {
                        if node_id * 3 + 2 < res.displacements.len() {
                            ...
                            pos[1] +=
                                res.displacements[node_id * 3 + 1] as f32 * displacement_scale;
                            pos[2] +=
                                res.displacements[node_id * 3 + 2] as f32 * displacement_scale;
                        }
                    }
                    positions.push(pos);
                }
            ...
            for (face_idx, face) in face_indices.iter().enumerate() {
                ...
                let node_ids = [conn[face[0]], conn[face[1]], conn[face[2]], conn[face[3]]];
                ...
                if let Some(res) = results {
                    ...
                    for (&node_id, _node) in &mesh.nodes {
                        if node_id * 3 + 2 < res.displacements.len() {
                            ...
                            self.colors
                                .node_colors
                                .insert(node_id, value_to_color(t.clamp(0.0, 1.0)));
                        }
                    }
                }
            }
            ColorMode::VonMises => {
                if let Some(res) = results {
                    ...
                    for (&el_id, &vm) in &res.von_mises {
                        ...
                        self.colors
                            .element_colors
                            .insert(el_id, value_to_color(t.clamp(0.0, 1.0)));
                    }
    ...
    pub fn get_face_color(&self, face: &CachedFace, color_mode: ColorMode) -> egui::Color32 {
        match color_mode {
            ...
            ColorMode::VonMises | ColorMode::Stress | ColorMode::Strain => self
                .colors
                .element_colors
                .get(&face.element_id)
                .copied()

...
use crate::state::{
    BoundaryConditionConfig, ContactBcConfig, FixedBcConfig, LoadBcConfig, SolverType,
    TorqueBcConfig,
};
...
pub fn load_example_with_config(config: &ExampleConfig) -> Example {
    ...
    match config.example_type {
        ExampleType::CantileverBeam => create_cantilever_beam(
            &params,
        ...
        ExampleType::TorqueShaft => {
            create_torque_shaft(&params, config.load_magnitude, config.scale)
        }
        ExampleType::ContactBlocks => {
            create_contact_blocks(&params, config.load_magnitude, config.scale)
        }
    }
}

/// Create a cantilever beam example
fn create_cantilever_beam(
    params: &ExampleMeshParams,
    load: f64,
    scale: f64,
...
fn create_cantilever_beam_c3d20(params: &ExampleMeshParams, load: f64, scale: f64) -> Example {
    ...
    let mut node_map: std::collections::HashMap<(usize, usize, usize), usize> =

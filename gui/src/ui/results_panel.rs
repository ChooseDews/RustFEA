...
pub fn show(ui: &mut egui::Ui, app: &mut FeaApp) {
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("stress_component")
                .selected_text(format!(
                    "σ_{}",
                    component_names[app.state.ui_state.stress_component]
                ))
                .show_ui(ui, |ui| {
    ...
    if app.state.ui_state.color_mode == ColorMode::Strain {
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("strain_component")
                .selected_text(format!(
                    "ε_{}",
                    component_names[app.state.ui_state.strain_component]
                ))
                .show_ui(ui, |ui| {
                    for (i, name) in component_names.iter().enumerate() {
                        if ui
        ...
        if ui.small_button("Auto").clicked() {
            ...
            if let Some(mesh) = app.state.current_mesh() {
                ...
                if max_disp > 0.0 {
                    ...
                    app.state.ui_state.displacement_scale = (0.1 * diag as f64 / max_disp) as f32;
                    app.renderer = None;
                }
            }
        }
    ...
    if time_steps_count > 0 {
        ...
        let current_idx = app
            ...
            .ui_state
            .current_time_step
            .min(time_steps_count.saturating_sub(1));
        ...
        ui.horizontal(|ui| {
            ...
            // Play/Pause
            let play_text = if app.state.ui_state.playback_active {
                "⏸"
            } else {
                "▶"
            };
            if ui
                .button(play_text)
                .on_hover_text(if app.state.ui_state.playback_active {
                    "Pause"
                } else {
                    "Play"
                })
            ...
            ui.label("Speed:");
            ui.add(
                egui::DragValue::new(&mut app.state.ui_state.playback_speed)
                    .range(1.0..=60.0)
                    .suffix(" fps"),
            );
        });
        ...
        if let Some(results) = &app.state.results {
            if results.time_steps.len() > 1 {
                ...
                let max_time = results.time_steps.last().map(|s| s.time).unwrap_or(1.0);

                // Draw displacement curve
                let points: Vec<egui::Pos2> = results
                    .time_steps
                    .iter()
                    .map(|step| {
                        let x = rect.left() + (step.time / max_time) as f32 * rect.width();
                        let y = rect.bottom()
                            - (step.max_displacement / max_disp) as f32 * rect.height() * 0.9;
                        egui::pos2(x, y)
                    })
                    .collect();
                ...
                if let Some(current_step) =
                    results.time_steps.get(app.state.ui_state.current_time_step)
                {
                    ...
                    .show(ui, |ui| {
                        egui::Grid::new("timesteps")
                            ...
                            .show(ui, |ui| {
                                ...
                                if let Some(results) = &app.state.results {
                                    for (idx, step) in
                                        results.time_steps.iter().enumerate().rev().take(50)
                                    {
                                        let is_current =
                                            idx == app.state.ui_state.current_time_step;
                                        if is_current {
...
fn generate_vtk_content(app: &FeaApp) -> Option<String> {
    ...
    let mut node_id_to_vtk: std::collections::HashMap<usize, usize> =
    ...
    let brick_elements: Vec<(&usize, &rust_fea::mesh::MeshElement)> = mesh
        .elements
        .iter()

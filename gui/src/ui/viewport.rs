    ...
    if response.clicked() {
        if let Some(pos) = response.interact_pointer_pos() {
            ...
            if let Some((elem_id, face_idx)) = find_face_at_position(pos, rect, app) {
                ...
                let remove_mode = response.ctx.input(|i| i.modifiers.ctrl || i.modifiers.command);
                ...
                app.state.status_message = format!("Selected {} faces", selection.selected_faces.len());
            }
        }
    }
}
...
fn find_face_at_position(pos: egui::Pos2, rect: egui::Rect, app: &FeaApp) -> Option<(usize, usize)> {
    ...
    for face in &cache.faces {
        ...
        let proj: Vec<Option<egui::Pos2>> = face.corners.iter()
...
fn show_viewport_context_menu(response: &egui::Response, app: &mut FeaApp) {
    response.context_menu(|ui| {
        ...
        ui.menu_button("👁 Display", |ui| {
            ...
            ui.checkbox(&mut app.state.ui_state.show_boundary_conditions, "Boundary Conditions");
            ui.separator();
            ui.checkbox(&mut app.state.ui_state.clipping_plane.enabled, "Clipping Plane");
        ...
        if app.state.current_mesh().is_some() {
            ui.menu_button("🎯 Selection", |ui| {
                ...
                if ui.checkbox(&mut app.state.ui_state.mesh_edit.face_selection.active, "Face Selection Mode").changed() {
            ...
            let selected_count = app.state.ui_state.mesh_edit.face_selection.selected_faces.len();
            if selected_count > 0 {
                ui.separator();
...
fn handle_camera_input(response: &egui::Response, app: &mut FeaApp) {
    ...
    if response.dragged_by(egui::PointerButton::Secondary) {
        ...
        camera.target[0] -= (cos_yaw * delta.x - sin_yaw * delta.y * camera.pitch.sin()) * pan_speed;
        camera.target[1] += delta.y * camera.pitch.cos() * pan_speed;
        camera.target[2] -= (sin_yaw * delta.x + cos_yaw * delta.y * camera.pitch.sin()) * pan_speed;
    }
...
fn draw_mesh_cached(
    ...
    if !cache.is_valid(mesh_state, results, scale, color_mode, stress_component, strain_component) {
        cache.rebuild(mesh_state, results, scale, color_mode, stress_component, strain_component);
    }
    ...
    for (i, &face_idx) in visible_face_indices.iter().enumerate() {
        ...
        let dot = face.normal[0] * view_dir[0] + face.normal[1] * view_dir[1] + face.normal[2] * view_dir[2];
        ...
        let proj = [
            view_transform.project(face.corners[0], rect_center, half_width, half_height, aspect),
            view_transform.project(face.corners[1], rect_center, half_width, half_height, aspect),
            view_transform.project(face.corners[2], rect_center, half_width, half_height, aspect),
            view_transform.project(face.corners[3], rect_center, half_width, half_height, aspect),
        ];
        ...
        if diag1_sq > max_screen_dist * max_screen_dist || 
           ...
           !diag1_sq.is_finite() || !diag2_sq.is_finite() {
            continue;
        }
    ...
    let section_cut_shapes = if clip_enabled && show_faces && ui_state.clipping_plane.show_section_surface {
    ...
    let mut depth_items: Vec<DepthSortedItem> = Vec::with_capacity(
        projected_faces.len() + section_cut_shapes.len()
    );
    ...
    if show_nodes {
        ...
        for (i, (_node_id, node)) in mesh.nodes.iter().enumerate() {
            ...
            if let Some(proj) = view_transform.project(pos, rect_center, half_width, half_height, aspect) {
        ...
        match bc {
            ...
            BoundaryConditionConfig::Load(cfg) => {
                if let Some(group) = mesh.node_groups.get(&cfg.node_group) {
                    ...
                    let force_mag = (cfg.force_x.powi(2) + cfg.force_y.powi(2) + cfg.force_z.powi(2)).sqrt();
                    if force_mag > 1e-10 {
                        let dir = [
                ...
                if let Some(group) = mesh.element_groups.get(&cfg.element_group) {
                    ...
                    for &el_id in &group.elements {
                        if let Some(element) = mesh.elements.get(&el_id) {
                            ...
                            let center = element.connectivity.iter()
            ...
            BoundaryConditionConfig::BodyForce(cfg) => {
                ...
                let dir = match &cfg.force_type {
                    ...
                    BodyForceTypeConfig::Centrifugal { axis_direction, .. } => {
                        // Show axis direction
                        let mag = (axis_direction[0].powi(2) + axis_direction[1].powi(2) + axis_direction[2].powi(2)).sqrt();
                        if mag > 1e-10 {
                            [(axis_direction[0] / mag) as f32, (axis_direction[1] / mag) as f32, (axis_direction[2] / mag) as f32]
                        } else {
                            [0.0, 1.0, 0.0]
                        }
                    }
                ...
                if let Some(proj) = project_point(center, rect, camera) {
                    ...
                    if rect.expand(50.0).contains(proj) {
                        ...
                        painter.text(proj, egui::Align2::CENTER_CENTER, label, 
...
fn draw_color_legend(painter: &egui::Painter, rect: egui::Rect, app: &FeaApp) {
    ...
    let (label, min_val, max_val) = match app.state.ui_state.color_mode {
        ...
        ColorMode::Displacement => {
            ("Displacement (m)", results.stats.min_displacement, results.stats.max_displacement)
        }
        ColorMode::VonMises => {
            ("Von Mises (Pa)", 0.0, results.stats.max_von_mises)
        }
        ColorMode::Stress => {
            let comp_names = ["σ_XX", "σ_YY", "σ_ZZ", "σ_XY", "σ_YZ", "σ_XZ"];
            let idx = app.state.ui_state.stress_component.min(5);
            let max_val = results.stresses.values()
        ...
        ColorMode::Strain => {
            ...
            let max_val = results.strains.values()
                .filter_map(|s| s.get(idx))
                .map(|v| v.abs())
                .fold(1e-10f64, |a, b| a.max(b));
            (comp_names[idx], -max_val, max_val)
    ...
    ctx.input(|i| {
        ...
        if app.state.results.is_some() {
            ...
            if i.key_pressed(egui::Key::ArrowRight) {
                let max_steps = app.state.results.as_ref()
...
fn show_clipping_controls(ui: &mut egui::Ui, app: &mut FeaApp) {
    ui.horizontal(|ui| {
        ...
        if ui.selectable_label(
            app.state.ui_state.clipping_plane.axis == ClipAxis::X, 
            "X"
        ).on_hover_text("Clip along X axis").clicked() {
            app.state.ui_state.clipping_plane.axis = ClipAxis::X;
            app.state.ui_state.clipping_plane.normal = ClipAxis::X.to_normal();
            changed = true;
        }
        if ui.selectable_label(
            app.state.ui_state.clipping_plane.axis == ClipAxis::Y, 
            "Y"
        ).on_hover_text("Clip along Y axis").clicked() {
            app.state.ui_state.clipping_plane.axis = ClipAxis::Y;
            app.state.ui_state.clipping_plane.normal = ClipAxis::Y.to_normal();
            changed = true;
        }
        if ui.selectable_label(
            app.state.ui_state.clipping_plane.axis == ClipAxis::Z, 
            "Z"
        ).on_hover_text("Clip along Z axis").clicked() {
            app.state.ui_state.clipping_plane.axis = ClipAxis::Z;
            app.state.ui_state.clipping_plane.normal = ClipAxis::Z.to_normal();
            changed = true;
        }
        ...
        // Flip direction
        if ui.checkbox(&mut app.state.ui_state.clipping_plane.flip, "Flip")
        ...
        if ui.checkbox(&mut app.state.ui_state.clipping_plane.show_section_surface, "Section Fill")
    ...
    egui::Window::new("⌨ Keyboard Shortcuts")
        ...
        .show(ctx, |ui| {
            ...
            egui::Grid::new("shortcuts_view")
                ...
                .spacing([20.0, 4.0])
                .show(ui, |ui| {
                    ui.strong("F"); ui.label("Toggle faces"); ui.end_row();
                    ui.strong("W"); ui.label("Toggle wireframe"); ui.end_row();
            ...
            egui::Grid::new("shortcuts_camera")
                ...
                .spacing([20.0, 4.0])
                .show(ui, |ui| {
                    ui.strong("1"); ui.label("Front view"); ui.end_row();
                    ui.strong("2"); ui.label("Side view"); ui.end_row();
                    ui.strong("3"); ui.label("Top view"); ui.end_row();
                    ui.strong("0"); ui.label("Isometric view"); ui.end_row();
                    ui.strong("Home"); ui.label("Fit to mesh"); ui.end_row();
            ...
            egui::Grid::new("shortcuts_clip")
                ...
                .spacing([20.0, 4.0])
                .show(ui, |ui| {
                    ui.strong("X"); ui.label("Clip along X axis"); ui.end_row();
                    ui.strong("Y"); ui.label("Clip along Y axis"); ui.end_row();
                    ui.strong("Z"); ui.label("Clip along Z axis"); ui.end_row();
                });
            ...
            egui::Grid::new("shortcuts_panels")
                ...
                .spacing([20.0, 4.0])
                .show(ui, |ui| {
                    ui.strong("Ctrl+1"); ui.label("Mesh panel"); ui.end_row();
                    ui.strong("Ctrl+2"); ui.label("Setup panel"); ui.end_row();
                    ui.strong("Ctrl+3"); ui.label("Run panel"); ui.end_row();
                    ui.strong("Ctrl+4"); ui.label("Results panel"); ui.end_row();
                });

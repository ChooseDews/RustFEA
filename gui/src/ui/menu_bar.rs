...
use crate::examples::{
    load_example, load_example_with_config, ExampleElementType, ExampleType, MeshResolution,
};
use crate::icons;
    ...
    egui::TopBottomPanel::top("menu_bar").show(ctx, |ui| {
        egui::menu::bar(ui, |ui| {
            ...
            ui.menu_button("File", |ui| {
                ...
                if ui
                    .button(format!("{} Save Project As...", icons::SAVE))
                    .clicked()
                {
                    save_project_as(app);
                    ui.close_menu();
                ...
                if ui
                    .button(format!("{} Import Mesh...", icons::FILE_ADD))
                    .clicked()
                {
                ...
                if !app.state.ui_state.recent_files.files.is_empty() {
                    ui.menu_button("Recent Files", |ui| {
                        ...
                        for recent in &app.state.ui_state.recent_files.files {
                            ...
                            if ui
                                .button(&label)
                                .on_hover_text(recent.path.to_string_lossy().to_string())
                                .clicked()
                            {
                ...
                ui.menu_button("Load Example", |ui| {
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("Cantilever Beam").clicked() {
                            app.state.ui_state.example_config.example_type =
                                crate::examples::ExampleType::CantileverBeam;
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("Torque Shaft").clicked() {
                            app.state.ui_state.example_config.example_type =
                                crate::examples::ExampleType::TorqueShaft;
                            app.state.ui_state.example_dialog_open = true;
                            ui.close_menu();
                        }
                    });
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("Contact Blocks").clicked() {
                            app.state.ui_state.example_config.example_type =
                                crate::examples::ExampleType::ContactBlocks;
                            app.state.ui_state.example_dialog_open = true;
                            ui.close_menu();
                        }
                    });
                ...
                if ui
                    .button(format!("{} Export Results...", icons::DOWNLOAD))
                    .clicked()
                {
                ...
                #[cfg(not(target_arch = "wasm32"))]
                if ui
                    .button(format!("{} Save Screenshot...", icons::SCREENSHOT))
                    .clicked()
                {
            ...
            ui.menu_button("Edit", |ui| {
                ...
                if ui
                    .button(format!("{} Preferences...", icons::SETTINGS))
                    .clicked()
                {
                ...
                if ui
                    .button(format!("{} Clear Workspace", icons::DELETE))
                    .on_hover_text("Remove all meshes and reset to default state")
                    .clicked()
                {
            ...
            ui.menu_button("View", |ui| {
                if ui
                    .checkbox(
                        &mut app.state.ui_state.show_faces,
                        format!("{} Show Faces", icons::FACES),
                    )
                    .on_hover_text("(F)")
                    .changed()
                {
                ...
                if ui
                    .checkbox(
                        &mut app.state.ui_state.show_wireframe,
                        format!("{} Show Wireframe", icons::WIREFRAME),
                    )
                    .on_hover_text("(W)")
                    .changed()
                {
                ...
                if ui
                    .checkbox(
                        &mut app.state.ui_state.show_nodes,
                        format!("{} Show Nodes", icons::POINTS),
                    )
                    .on_hover_text("(N)")
                    .changed()
                {
                ...
                ui.checkbox(
                    &mut app.state.ui_state.show_node_groups,
                    format!("{} Show Node Groups", icons::NODE_TREE),
                );
                ui.checkbox(
                    &mut app.state.ui_state.show_boundary_conditions,
                    format!("{} Show Boundary Conditions", icons::MARKUP),
                )
                .on_hover_text("(B)");
                ...
                if ui
                    .checkbox(
                        ...
                        format!("{} Clipping Plane", icons::SCISSORS),
                    )
                ...
                if ui
                    .button(format!("{} Reset Camera", icons::FOCUS))
                    .on_hover_text("(Home)")
                    .clicked()
                {
                ...
                if ui
                    .button(format!("{} Front View", icons::VIEW_FRONT))
                    .on_hover_text("(1)")
                    .clicked()
                {
                ...
                if ui
                    .button(format!("{} Top View", icons::VIEW_TOP))
                    .on_hover_text("(3)")
                    .clicked()
                {
                ...
                if ui
                    .button(format!("{} Side View", icons::VIEW_LEFT))
                    .on_hover_text("(2)")
                    .clicked()
                {
                ...
                if ui
                    .button(format!("{} Isometric", icons::VIEW_ISO))
                    .on_hover_text("(0)")
                    .clicked()
                {
                ...
                if ui
                    .button(format!("{} Keyboard Shortcuts...", icons::QUESTION))
                    .on_hover_text("(?)")
                    .clicked()
                {
            ...
            ui.menu_button("Simulation", |ui| {
                ...
                ui.add_enabled_ui(can_run, |ui| {
                    if ui
                        .button(format!("{} Run Simulation", icons::PLAY))
                        .clicked()
                    {
                ...
                ui.add_enabled_ui(app.state.is_running, |ui| {
                    if ui
                        .button(format!("{} Stop Simulation", icons::STOP))
                        .clicked()
                    {
                        app.stop_simulation();
                        ui.close_menu();
                ...
                if ui
                    .button(format!("{} Solver Settings...", icons::SETTINGS))
                    .clicked()
                {
            ...
            ui.menu_button("Help", |ui| {
                ...
                if ui
                    .button(format!("{} Documentation", icons::EXTERNAL_LINK))
                    .clicked()
                {
        ...
        Ok((project, config)) => {
            ...
            if let Some(mesh_state) = app.state.current_mesh() {
                ...
                app.state.status_message =
                    format!("Project loaded: {} (no mesh specified)", project.name);
            }
...
fn download_project_as(app: &mut FeaApp, filename: &str) {
    ...
    let toml_str =
        match project_io::serialize_project_toml(&mesh_name, None, &app.state.simulation_config) {
            Ok(s) => s,
            Err(e) => {
    ...
    let mesh_json = app
        .state
        .current_mesh()
        .map(|m| serde_json::to_string_pretty(&m.mesh).ok())
        .flatten();

    // Create and download ZIP bundle
    let project_name = filename.trim_end_matches(".toml").trim_end_matches(".rfea");
    crate::web_file_io::download_project_bundle(project_name, &toml_str, mesh_json.as_deref());
...
fn do_save_project(app: &mut FeaApp, path: &std::path::Path) {
    let mesh_path = app
        .state
        .current_mesh()
        .and_then(|m| m.path.as_ref().map(|p| p.as_path()));
    let name = app
        .state
        .current_mesh()
        .map(|m| m.name.as_str())
...
fn export_results_dialog(app: &mut FeaApp) {
  ...
  "num_time_steps": {}
            ...
            crate::web_file_io::download_file(
...
fn load_example_into_app(app: &mut FeaApp, example_type: ExampleType) {
    ...
    let mesh_state = crate::state::MeshState::from_mesh(example.mesh, example.name.clone(), None);
...
pub fn show_example_dialog(ctx: &egui::Context, app: &mut FeaApp) {
    ...
    egui::Window::new("Load Example")
        ...
        .show(ctx, |ui| {
            ...
            let (est_nodes, est_elements) =
                estimate_mesh_size(config.example_type, config.element_type, &params);
            ui.label(format!(
                "Estimated: ~{} nodes, ~{} elements",
                est_nodes, est_elements
            ));
            ...
            if config.resolution == MeshResolution::Custom {
                ui.collapsing("Custom Mesh Parameters", |ui| match config.example_type {
                    ExampleType::CantileverBeam => {
                        egui::Grid::new("beam_params")
                            ...
                            .show(ui, |ui| {
                                ui.label("Length (m):");
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.beam_length)
                                        .speed(0.1),
                                );
                                ui.end_row();
                                ...
                                ui.label("Height (m):");
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.beam_height)
                                        .speed(0.1),
                                );
                                ui.end_row();
                                ...
                                ui.label("Width (m):");
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.beam_width)
                                        .speed(0.1),
                                );
                                ui.end_row();
                                ...
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.beam_nx)
                                        .range(2..=100),
                                );
                                ui.end_row();
                                ...
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.beam_ny)
                                        .range(1..=20),
                                );
                                ui.end_row();
                                ...
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.beam_nz)
                                        .range(1..=20),
                                );
                                ui.end_row();
                            });
                    ...
                    ExampleType::TorqueShaft => {
                        egui::Grid::new("shaft_params")
                            ...
                            .show(ui, |ui| {
                                ui.label("Radius (m):");
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.shaft_radius)
                                        .speed(0.1),
                                );
                                ui.end_row();
                                ...
                                ui.label("Length (m):");
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.shaft_length)
                                        .speed(0.1),
                                );
                                ui.end_row();
                                ...
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.shaft_n_radial)
                                        .range(4..=32),
                                );
                                ui.end_row();
                                ...
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.shaft_n_height)
                                        .range(2..=50),
                                );
                                ui.end_row();
                                ...
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.shaft_n_layers)
                                        .range(1..=8),
                                );
                                ui.end_row();
                            });
                    ...
                    ExampleType::ContactBlocks => {
                        egui::Grid::new("contact_params")
                            ...
                            .show(ui, |ui| {
                                ...
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.block_size)
                                        .speed(0.1),
                                );
                                ui.end_row();
                                ...
                                ui.label("Gap (m):");
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.block_gap)
                                        .speed(0.001),
                                );
                                ui.end_row();
                                ...
                                ui.label("Divisions:");
                                ui.add(
                                    egui::DragValue::new(&mut config.custom_params.block_divisions)
                                        .range(1..=20),
                                );
                                ui.end_row();
                            });
...
fn estimate_mesh_size(
    example_type: ExampleType,
    element_type: ExampleElementType,
    params: &crate::examples::ExampleMeshParams,
) -> (usize, usize) {
    match example_type {
        ExampleType::CantileverBeam => {
        ...
        ExampleType::TorqueShaft => {
            ...
            let layer_nodes =
                params.shaft_n_layers * params.shaft_n_radial * (params.shaft_n_height + 1);
            ...
            UndoAction::AddBoundaryCondition(bc) => {
                ...
                if let Some(idx) = app
                    ...
                    .position(|b| {
                        ...
                        match (b, bc) {
                            (
                                crate::state::BoundaryConditionConfig::Fixed(a),
                                crate::state::BoundaryConditionConfig::Fixed(b),
                            ) => a.name == b.name,
                            (
                                crate::state::BoundaryConditionConfig::Load(a),
                                crate::state::BoundaryConditionConfig::Load(b),
                            ) => a.name == b.name,
                            (
                                crate::state::BoundaryConditionConfig::Torque(a),
                                crate::state::BoundaryConditionConfig::Torque(b),
                            ) => a.name == b.name,
                            (
                                crate::state::BoundaryConditionConfig::Contact(a),
                                crate::state::BoundaryConditionConfig::Contact(b),
                            ) => a.name == b.name,
                            (
                                crate::state::BoundaryConditionConfig::Pressure(a),
                                crate::state::BoundaryConditionConfig::Pressure(b),
                            ) => a.name == b.name,
                            (
                                crate::state::BoundaryConditionConfig::Traction(a),
                                crate::state::BoundaryConditionConfig::Traction(b),
                            ) => a.name == b.name,
                            (
                                crate::state::BoundaryConditionConfig::BodyForce(a),
                                crate::state::BoundaryConditionConfig::BodyForce(b),
                            ) => a.name == b.name,
                            _ => false,
                        }
                    })

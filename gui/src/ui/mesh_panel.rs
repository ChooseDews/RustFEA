...
use crate::state::{NodeSelectionMode, PrimitiveType};
use eframe::egui;
    ...
    if app.state.meshes.is_empty() {
        ...
        if let Some(idx) = remove_idx {
            ...
            if app.state.meshes.is_empty() {
                ...
                app.state.current_mesh_idx =
                    Some(idx.saturating_sub(1).min(app.state.meshes.len() - 1));
        ...
        let (mesh_name, mesh_path, node_count, elem_count, body_count, bounds, element_groups) = {
            ...
            (
                ...
                mesh.mesh
                    ...
                    .iter()
                    .map(|(name, group)| {
                        (name.clone(), group.elements.len(), group.el_type.clone())
                    })
                    .collect::<Vec<_>>(),
            )
        };
        ...
        if !element_groups.is_empty() {
            ...
            ui.collapsing("Element Groups", |ui| {
                for (name, count, el_type) in &element_groups {
                    ui.horizontal(|ui| {
                        ui.label(format!("🧊 {} ({} elements, {})", name, count, el_type));
                    });
                }
            });
        }
    ...
    ui.collapsing("Transform", |ui| {
        ...
        match transform.mode {
            ...
            TransformMode::Scale => {
                ...
                ui.horizontal(|ui| {
                    if ui.small_button("Uniform").clicked() {
                        let avg =
                            (transform.scale[0] + transform.scale[1] + transform.scale[2]) / 3.0;
            ...
            TransformMode::Rotate => {
                ...
                ui.horizontal(|ui| {
                    if ui.small_button("+90° X").clicked() {
                        transform.rotation[0] += 90.0;
                    }
                    if ui.small_button("+90° Y").clicked() {
                        transform.rotation[1] += 90.0;
                    }
                    if ui.small_button("+90° Z").clicked() {
...
fn show_face_selection_controls(ui: &mut egui::Ui, app: &mut FeaApp) {
    ...
    ui.collapsing("🎯 Face Selection", |ui| {
        ...
        if selection.active {
            ...
            ui.colored_label(
                egui::Color32::GRAY,
                "Shift+click to add, Ctrl+click to remove",
            );
        }
    });
    ...
    if let Some(mesh_idx) = app.state.current_mesh_idx {
        if let Some(mesh_state) = app.state.meshes.get_mut(mesh_idx) {
            ...
            app.state.status_message =
                format!("Created node group '{}' from {} faces", name, faces.len());
        }
    }
...
fn create_bc_from_faces(app: &mut FeaApp, bc_type: &str) {
    ...
    if let Some(name) = group_name {
        match bc_type {
            "fixed" => {
                let bc =
                    crate::state::BoundaryConditionConfig::Fixed(crate::state::FixedBcConfig {
            ...
            "load" => {
                ...
                app.state.simulation_config.boundary_conditions.push(bc);
                app.state.status_message =
                    "Created Load BC from face selection (edit in Setup)".to_string();
            }
            _ => {}
        }
...
fn show_node_groups_section(ui: &mut egui::Ui, app: &mut FeaApp) {
    ...
    ui.collapsing("Node Groups", |ui| {
        ...
        if has_groups {
            ...
            let groups_info: Vec<(String, usize)> = app
                .state
                .current_mesh()
                .map(|m| {
                    m.mesh
            ...
            for (name, node_count) in &groups_info {
                let response = ui
                    .horizontal(|ui| ui.label(format!("📍 {} ({} nodes)", name, node_count)))
                    .response;
            ...
            if let Some((name, act)) = action {
                match act {
                    ...
                    "fixed" => {
                        let bc = crate::state::BoundaryConditionConfig::Fixed(
                            crate::state::FixedBcConfig {
                                name: format!("Fixed_{}", name),
                    ...
                    "load" => {
                        let bc = crate::state::BoundaryConditionConfig::Load(
                            crate::state::LoadBcConfig {
                                name: format!("Load_{}", name),
                                node_group: name.clone(),
                                force_x: 0.0,
                                force_y: -1000.0,
                        ...
                        app.state.simulation_config.boundary_conditions.push(bc);
                        app.state.status_message =
                            format!("Applied Load to '{}' - edit in Setup", name);
                    }
                    _ => {}
                }
            }
        ...
        if ui.button("+ Create Node Group").clicked() {
            ...
            app.state.ui_state.node_group_creator.name = format!(
                "Group_{}",
                app.state
                    .current_mesh()
                    .map(|m| m.mesh.node_groups.len())
                    .unwrap_or(0)
                    + 1
            );
        }
...
fn show_box_selection_ui(ui: &mut egui::Ui, app: &mut FeaApp) {
    ...
    ui.horizontal(|ui| {
        ui.add(
            egui::DragValue::new(&mut creator.box_min[0])
                .prefix("X: ")
                .speed(0.1),
        );
        ui.add(
            egui::DragValue::new(&mut creator.box_min[1])
                .prefix("Y: ")
                .speed(0.1),
        );
        ui.add(
            egui::DragValue::new(&mut creator.box_min[2])
                .prefix("Z: ")
                .speed(0.1),
        );
    });
    ...
    ui.horizontal(|ui| {
        ui.add(
            egui::DragValue::new(&mut creator.box_max[0])
                .prefix("X: ")
                .speed(0.1),
        );
        ui.add(
            egui::DragValue::new(&mut creator.box_max[1])
                .prefix("Y: ")
                .speed(0.1),
        );
        ui.add(
            egui::DragValue::new(&mut creator.box_max[2])
                .prefix("Z: ")
                .speed(0.1),
        );
    });
    ...
    egui::Window::new("Create Primitive Mesh")
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .button("Create & Edit")
                    .on_hover_text("Create and keep dialog open for adjustments")
                    .clicked()
                {
                    create_primitive_mesh(app);
...
fn show_block_config_enhanced(ui: &mut egui::Ui, config: &mut crate::state::PrimitiveConfig) {
    ...
    ui.group(|ui| {
        ...
        ui.horizontal(|ui| {
            ui.label("Width (X):");
            ui.add(
                egui::DragValue::new(&mut config.block_size[0])
                    .speed(0.1)
                    .range(0.01..=1000.0),
            );
        });
        ui.horizontal(|ui| {
            ui.label("Height (Y):");
            ui.add(
                egui::DragValue::new(&mut config.block_size[1])
                    .speed(0.1)
                    .range(0.01..=1000.0),
            );
        });
        ui.horizontal(|ui| {
            ui.label("Depth (Z):");
            ui.add(
                egui::DragValue::new(&mut config.block_size[2])
                    .speed(0.1)
                    .range(0.01..=1000.0),
            );
        });
    ...
    ui.group(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(egui::Slider::new(&mut div_x, 1..=20).show_value(true))
                .changed()
            {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(egui::Slider::new(&mut div_y, 1..=20).show_value(true))
                .changed()
            {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(egui::Slider::new(&mut div_z, 1..=20).show_value(true))
                .changed()
            {
        ...
        ui.colored_label(
            ...
            format!(
                "Element size: {:.3} × {:.3} × {:.3}",
                elem_size_x, elem_size_y, elem_size_z
            ),
        );
    });
    ...
    ui.collapsing("Origin / Position", |ui| {
        ui.horizontal(|ui| {
            ui.add(
                egui::DragValue::new(&mut config.block_origin[0])
                    .prefix("X: ")
                    .speed(0.1),
            );
            ui.add(
                egui::DragValue::new(&mut config.block_origin[1])
                    .prefix("Y: ")
                    .speed(0.1),
            );
            ui.add(
                egui::DragValue::new(&mut config.block_origin[2])
                    .prefix("Z: ")
                    .speed(0.1),
            );
        });
    ...
    ui.add_space(8.0);
    let num_elements =
        config.block_divisions[0] * config.block_divisions[1] * config.block_divisions[2];
    let num_nodes = (config.block_divisions[0] + 1)
        * (config.block_divisions[1] + 1)
        * (config.block_divisions[2] + 1);
    ui.separator();
    ui.colored_label(
...
fn show_cylinder_config_enhanced(ui: &mut egui::Ui, config: &mut crate::state::PrimitiveConfig) {
    ...
    ui.group(|ui| {
        ...
        ui.horizontal(|ui| {
            ui.label("Radius:");
            ui.add(
                egui::DragValue::new(&mut config.cyl_radius)
                    .speed(0.01)
                    .range(0.01..=1000.0),
            );
        });
        ui.horizontal(|ui| {
            ui.label("Height:");
            ui.add(
                egui::DragValue::new(&mut config.cyl_height)
                    .speed(0.1)
                    .range(0.01..=1000.0),
            );
        });
    ...
    ui.group(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(egui::Slider::new(&mut radial, 4..=32).show_value(true))
                .changed()
            {
        ...
        ui.horizontal(|ui| {
            ui.label("Axial:");
            if ui
                .add(egui::Slider::new(&mut height, 1..=20).show_value(true))
                .changed()
            {
    ...
    ui.collapsing("Axis & Position", |ui| {
        ...
        ui.horizontal(|ui| {
            if ui.small_button("X").clicked() {
                config.cyl_axis = [1.0, 0.0, 0.0];
            }
            if ui.small_button("Y").clicked() {
                config.cyl_axis = [0.0, 1.0, 0.0];
            }
            if ui.small_button("Z").clicked() {
                config.cyl_axis = [0.0, 0.0, 1.0];
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.add(
                egui::DragValue::new(&mut config.cyl_origin[0])
                    .prefix("X: ")
                    .speed(0.1),
            );
            ui.add(
                egui::DragValue::new(&mut config.cyl_origin[1])
                    .prefix("Y: ")
                    .speed(0.1),
            );
            ui.add(
                egui::DragValue::new(&mut config.cyl_origin[2])
                    .prefix("Z: ")
                    .speed(0.1),
            );
        });
...
fn create_cylinder_mesh(config: &crate::state::PrimitiveConfig) -> rust_fea::mesh::MeshAssembly {
    ...
    // Bottom face
    let bottom_nodes: Vec<usize> = node_ids[0]
        .iter()
    ...
    // Top face
    let top_nodes: Vec<usize> = node_ids[n_height]
        .iter()
        .flat_map(|[inner, outer]| vec![*inner, *outer])
        .collect();
    node_groups.insert(
    ...
    // Inner surface
    let inner_nodes: Vec<usize> = node_ids
        .iter()
    ...
    // Outer surface
    let outer_nodes: Vec<usize> = node_ids
        .iter()

...
use crate::state::{
    ...
    FixedBcConfig, LoadBcConfig, MaterialConfig, NewBcType, PressureBcConfig, SolverType,
    TorqueBcConfig, TractionBcConfig, TractionTypeConfig,
};
use eframe::egui;
...
fn show_solver_settings(ui: &mut egui::Ui, app: &mut FeaApp) {
    ...
    ui.horizontal(|ui| {
        ...
        if ui
            .add(egui::DragValue::new(&mut dofs).range(1..=6))
            .changed()
        {
    ...
    if config.solver == SolverType::Explicit {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(egui::DragValue::new(&mut steps).range(1..=1000000))
                .changed()
            {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(egui::DragValue::new(&mut interval).range(1..=100000))
                .changed()
            {
            ...
            ui.group(|ui| {
                ...
                egui::Grid::new("material_props")
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.add(
                            egui::DragValue::new(&mut material.youngs_modulus)
                                .speed(1e9)
                                .range(1e3..=1e15),
                        );
                        ui.end_row();
                        ...
                        ui.add(
                            egui::DragValue::new(&mut material.poissons_ratio)
                                .speed(0.01)
                                .range(0.0..=0.5),
                        );
                        ui.end_row();
                        ...
                        ui.add(
                            egui::DragValue::new(&mut material.density)
                                .speed(100.0)
                                .range(1.0..=50000.0),
                        );
                        ui.end_row();
...
fn show_boundary_conditions(ui: &mut egui::Ui, app: &mut FeaApp) {
    ...
    let node_groups: Vec<String> = app
        .state
        .current_mesh()
        .map(|m| m.mesh.node_groups.keys().cloned().collect())
        .unwrap_or_default();

    let element_groups: Vec<String> = app
        .state
        .current_mesh()
    ...
    ui.horizontal(|ui| {
        ...
        egui::ComboBox::from_id_salt("new_bc_type")
            ...
            .show_ui(ui, |ui| {
                ui.selectable_value(
                    &mut app.state.ui_state.bc_editor.new_bc_type,
                    NewBcType::Fixed,
                    "Fixed",
                );
                ui.selectable_value(
        ...
        if ui.button("+").clicked() {
            let new_bc = match app.state.ui_state.bc_editor.new_bc_type {
                ...
                NewBcType::Pressure => {
                    BoundaryConditionConfig::Pressure(PressureBcConfig::default())
                }
                NewBcType::Traction => {
                    BoundaryConditionConfig::Traction(TractionBcConfig::default())
                }
                NewBcType::BodyForce => {
                    BoundaryConditionConfig::BodyForce(BodyForceBcConfig::default())
                }
            };
...
fn show_torque_bc(
    ...
    egui::Grid::new("torque_bc_grid")
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.add(
                    egui::DragValue::new(&mut cfg.axis_point[0])
                        .prefix("x: ")
                        .speed(0.1),
                );
                ui.add(
                    egui::DragValue::new(&mut cfg.axis_point[1])
                        .prefix("y: ")
                        .speed(0.1),
                );
                ui.add(
                    egui::DragValue::new(&mut cfg.axis_point[2])
                        .prefix("z: ")
                        .speed(0.1),
                );
            });
            ...
            ui.horizontal(|ui| {
                ui.add(
                    egui::DragValue::new(&mut cfg.axis_direction[0])
                        .prefix("x: ")
                        .speed(0.1),
                );
                ui.add(
                    egui::DragValue::new(&mut cfg.axis_direction[1])
                        .prefix("y: ")
                        .speed(0.1),
                );
                ui.add(
                    egui::DragValue::new(&mut cfg.axis_direction[2])
                        .prefix("z: ")
                        .speed(0.1),
                );
            });
...
fn show_pressure_bc(
    ...
    egui::Grid::new("pressure_bc_grid")
        ...
        .show(ui, |ui| {
            ...
            egui::ComboBox::from_id_salt("pressure_element_group")
                .selected_text(if cfg.element_group.is_empty() {
                    "(select)"
                } else {
                    &cfg.element_group
                })
    ...
    // Preset buttons
    ui.horizontal(|ui| {
        if ui.small_button("1 kPa").clicked() {
            cfg.pressure = 1e3;
        }
        if ui.small_button("1 MPa").clicked() {
            cfg.pressure = 1e6;
        }
        if ui.small_button("100 MPa").clicked() {
            cfg.pressure = 100e6;
        }
    });
}
        ...
        .show(ui, |ui| {
            ...
            egui::ComboBox::from_id_salt("traction_type")
                ...
                .show_ui(ui, |ui| {
                    if ui
                        .selectable_label(
                            matches!(&cfg.traction_type, TractionTypeConfig::Uniform { .. }),
                            "Uniform",
                        )
                        .clicked()
                    {
                        cfg.traction_type = TractionTypeConfig::Uniform {
                            fx: 0.0,
                            fy: 0.0,
                            fz: -1e6,
                        };
                    ...
                    if ui
                        .selectable_label(
                            matches!(&cfg.traction_type, TractionTypeConfig::Normal { .. }),
                            "Normal",
                        )
                        .clicked()
                    {
                    ...
                    if ui
                        .selectable_label(
                            matches!(&cfg.traction_type, TractionTypeConfig::Shear { .. }),
                            "Shear",
                        )
                        .clicked()
                    {
...
fn show_body_force_bc(
        ...
        .show(ui, |ui| {
            ...
            egui::ComboBox::from_id_salt("bodyforce_type")
                ...
                .show_ui(ui, |ui| {
                    if ui
                        .selectable_label(
                            matches!(&cfg.force_type, BodyForceTypeConfig::Gravity { .. }),
                            "Gravity",
                        )
                        .clicked()
                    {
                        cfg.force_type = BodyForceTypeConfig::Gravity {
                            gx: 0.0,
                            gy: -9.81,
                            gz: 0.0,
                        };
                    ...
                    if ui
                        .selectable_label(
                            matches!(&cfg.force_type, BodyForceTypeConfig::Centrifugal { .. }),
                            "Centrifugal",
                        )
                        .clicked()
                    {
                    ...
                    if ui
                        .selectable_label(
                            matches!(&cfg.force_type, BodyForceTypeConfig::Uniform { .. }),
                            "Uniform",
                        )
                        .clicked()
                    {
                        cfg.force_type = BodyForceTypeConfig::Uniform {
                            fx: 0.0,
                            fy: 0.0,
                            fz: 0.0,
                        };
    ...
    match &mut cfg.force_type {
        BodyForceTypeConfig::Gravity { gx, gy, gz } => {
            ...
            // Preset buttons
            ui.horizontal(|ui| {
                if ui.small_button("-Y (Earth)").clicked() {
                    *gx = 0.0;
                    *gy = -9.81;
                    *gz = 0.0;
                }
                if ui.small_button("-Z (Earth)").clicked() {
                    *gx = 0.0;
                    *gy = 0.0;
                    *gz = -9.81;
                }
        ...
        BodyForceTypeConfig::Centrifugal {
            ...
            egui::Grid::new("centrifugal_grid")
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.add(
                            egui::DragValue::new(&mut axis_point[0])
                                .prefix("x: ")
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut axis_point[1])
                                .prefix("y: ")
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut axis_point[2])
                                .prefix("z: ")
                                .speed(0.1),
                        );
                    });
                    ...
                    ui.horizontal(|ui| {
                        ui.add(
                            egui::DragValue::new(&mut axis_direction[0])
                                .prefix("x: ")
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut axis_direction[1])
                                .prefix("y: ")
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut axis_direction[2])
                                .prefix("z: ")
                                .speed(0.1),
                        );
                    });

...
pub fn eular_beam_displacement(
    x: f64,
    length: f64,
    load: f64,
    modulus: f64,
...
impl Keywords {
    ...
    pub fn get_keyword(&self, keyword: &str) -> Option<&Keyword> {
        self.values
            .iter()
            .find(|k| k.keyword == keyword.to_uppercase())
    }
    pub fn keyword_exists(&self, keyword: &str) -> bool {
        self.values
            .iter()
            .any(|k| k.keyword == keyword.to_uppercase())
    }
    pub fn get_keywords(&self, keyword: &str) -> Vec<&Keyword> {
        //some keywords may have multiple values or instances
        self.values
            .iter()
            .filter(|k| k.keyword == keyword.to_uppercase())
            .collect()
    }

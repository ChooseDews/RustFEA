...
impl BoundaryCondition for PressureCondition {
    fn initalize(&mut self, simulation: &Simulation) {
        debug!(
            "Initializing pressure condition with {} elements",
            self.elements.len()
        );
        ...
        for &elem_id in &self.elements {
            ...
            if connectivity.len() != 4 {
                debug!(
                    "Warning: Pressure BC requires 4-node surface elements, got {} nodes",
                    connectivity.len()
                );
                continue;
            }
        ...
        debug!(
            "Pressure BC initialized: {} nodes, total force magnitude: {:.6e}",
            self.nodes.len(),
...
impl fmt::Display for PressureCondition {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "PressureCondition: {} elements, {} nodes, P={:.3e}",
            self.elements.len(),
            self.nodes.len(),
            self.pressure
        )

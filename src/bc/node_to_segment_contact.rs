...
impl NodeToSegmentContact {
    ...
    fn find_closest_point_on_segment(
        ...
        let center =
            (element_nodes[0] + element_nodes[1] + element_nodes[2] + element_nodes[3]) / 4.0;
    ...
    fn apply_penalty_contact(&self, penetration: f64, normal: &Vector3<f64>) -> Vector3<f64> {
        if penetration > 0.0 {
...
impl BoundaryCondition for NodeToSegmentContact {
    ...
    fn initalize(&mut self, simulation: &Simulation) {
        ...
        debug!(
            "Contact initialized: {} active primary nodes, {} secondary elements",
            self.active_primary_nodes.len(),
            self.active_secondary_elements.len()
        );
    }

    fn print_stats(&self) {
        debug!(
            "Contact stats: max_pen={:.6e}, active={}, max_friction={:.6e}",

...
use crate::mesh::{ElementGroup, MeshAssembly, MeshElement, MeshNode};
...
pub fn run() -> BenchmarkResult {
    ...
    for (nx, ny, nz, mesh_name) in refinements {
        info!(
            "Running C3D4 uniaxial tension benchmark with {} mesh ({} x {} x {} hex cells)",
            mesh_name, nx, ny, nz
        );
...
mod tests {
    ...
    fn test_c3d4_uniaxial_tension_benchmark() {
        ...
        assert!(
            result.passed,
            "C3D4 uniaxial tension benchmark failed: {:?}",
            result.metrics
        );
    }
    ...
    fn test_c3d4_mesh_generation() {
        ...
        assert_eq!(
            mesh.elements.len(),
            6,
            "Expected 6 elements for 1x1x1 C3D4 mesh"
        );
        ...
        for (_, elem) in &mesh.elements {
            assert_eq!(
                ...
                4,
                "C3D4 element should have 4 nodes"
            );
        }

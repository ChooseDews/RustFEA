...
use crate::bc::{FixedCondition, BodyForce};
...
pub fn run() -> BenchmarkResult {
    ...
    simulation.add_boundary_condition(Box::new(
        FixedCondition::new(fixed_nodes, vec![Some(0.0), Some(0.0), Some(0.0)])
    ));
    
    // Apply gravity body force (negative Z direction)
    simulation.add_boundary_condition(Box::new(
        BodyForce::gravity_from_vec(0.0, 0.0, -g)
    ));
    ...
    result.set_notes(&format!(
        "Beam: {}m × {}m × {}m, Aluminum (E={:.1e} Pa, ρ={} kg/m³)\n\
         ...
         FEA δ_max = {:.4e} m\n\
         Error: {:.2}%",
        length, width, height, e, density, q,
        delta_analytical, max_z_disp, error
    ));

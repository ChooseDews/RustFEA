...
pub fn run() -> BenchmarkResult {
    ...
    info!("  Analytical twist angle (square): {:.6} rad = {:.4}°", 
          twist_angle_square, twist_angle_square.to_degrees());
    
    // Run with medium mesh - explicit is slower so use coarser mesh
    let (nx, ny, nz) = (20, 4, 4);
    info!("Running explicit solver ({} x {} x {} elements)", nx, ny, nz);
    ...
    info!("  Direct twist:   {:.6} rad ({:.2}% error vs analytical)", 
          direct_twist, (direct_twist - twist_angle_square).abs() / twist_angle_square * 100.0);
    info!("  Explicit twist: {:.6} rad ({:.2}% error vs analytical)", 
          explicit_twist, (explicit_twist - twist_angle_square).abs() / twist_angle_square * 100.0);
    info!("  Explicit vs Direct error: {:.2}%", explicit_vs_direct_error * 100.0);
    ...
    result.set_notes(&format!(
        ...
        actual_e, actual_nu, g, L, SIDE, T, nx, ny, nz, twist_angle_square.to_degrees()
    ));

...
pub fn run() -> BenchmarkResult {
    ...
    info!("  Domain: {} × {} × {} m block", DOMAIN_SIZE, DOMAIN_SIZE, DOMAIN_SIZE);
    ...
    for (nx, ny, nz, mesh_name) in mesh_configs {
        ...
        let error = mesh_result.metrics.iter()
...
fn run_single_mesh(
    ...
    let center_node_candidates: Vec<usize> = z_max_nodes.iter()
        .filter(|&&id| {
    ...
    for (depth, label) in test_configs {
        ...
        let nearby_nodes: Vec<_> = nodes.iter()
            .filter(|n| {
        ...
        let avg_u_z: f64 = nearby_nodes.iter()
            .map(|n| n.displacement.z)
            .sum::<f64>() / nearby_nodes.len() as f64;
    ...
    let surface_nodes: Vec<_> = nodes.iter()
        .filter(|n| (n.position.z - DOMAIN_SIZE).abs() < dx / 2.0)
        .collect();
    
    // Sample at r = 0.2m from center
    let r_test = 0.2;
    let nearby_surface: Vec<_> = surface_nodes.iter()
        .filter(|n| {
            let r = (n.position.x.powi(2) + n.position.y.powi(2)).sqrt();
            (r - r_test).abs() < dx
        })
    ...
    if !nearby_surface.is_empty() {
        let avg_surface_u_z: f64 = nearby_surface.iter()
            .map(|n| n.displacement.z)
            .sum::<f64>() / nearby_surface.len() as f64;

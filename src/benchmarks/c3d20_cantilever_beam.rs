...
pub fn run() -> BenchmarkResult {
    ...
    for (nx, ny, nz, mesh_name) in refinements {
        info!("Running {} mesh ({} x {} x {} = {} elements)", 
              mesh_name, nx, ny, nz, nx * ny * nz);
...
fn run_single_mesh(n_x: usize, n_y: usize, n_z: usize, delta_analytical: f64) -> BenchmarkResult {
    ...
    info!("  Mesh: {} nodes, {} elements", mesh.nodes.len(), mesh.elements.len());
    info!("  Fixed end nodes: {}, Load end nodes: {}", x_min_nodes.len(), x_max_nodes.len());
    ...
    info!("  Computed tip deflection: {:.6e} m (analytical: {:.6e} m)", 
    ...
    info!("  Stiffness ratio: {:.3} (1.0 = perfect, >1 = too stiff)", stiffness_ratio);
...
mod tests {
    ...
    fn test_c3d20_cantilever_beam() {
        ...
        for metric in &result.metrics {
            ...
            println!("  {} [{}]: analytical={:.4e}, computed={:.4e}, error={:.2}%",
                     metric.name, status, metric.analytical, metric.computed, 
                     metric.relative_error.abs() * 100.0);
        }
        
        assert!(result.passed, 
            "C3D20 cantilever beam benchmark failed. Metrics: {:?}", result.metrics);
    }
    ...
    fn test_c3d20_cantilever_single_element() {
        ...
        println!("Single element mesh: {} nodes, {} elements", 
                 mesh.nodes.len(), mesh.elements.len());
        println!("Fixed nodes: {}, Load nodes: {}", x_min_nodes.len(), x_max_nodes.len());
        ...
        let nodes = sim.nodes();
        let tip_z: Vec<f64> = x_max_nodes.iter()
            .filter_map(|&id| nodes.get(id))
        ...
        assert!(avg_tip < 0.0, "Expected downward (negative) deflection, got {}", avg_tip);
    }
}

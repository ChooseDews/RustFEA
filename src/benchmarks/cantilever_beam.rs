...
pub fn run() -> BenchmarkResult {
    ...
    let slenderness_ratios = vec![(5.0, "L/h=5"), (10.0, "L/h=10"), (20.0, "L/h=20")];
    ...
    for (ratio, label) in slenderness_ratios {
        ...
        info!(
            "Running cantilever benchmark: {} (L={:.2} m)",
            label, length
        );
    ...
    let mesh_refinements = vec![
        ...
        (20, 2, 2, "medium", false),
        (40, 4, 4, "fine", false),
        (60, 6, 6, "very_fine", false),
        (80, 8, 8, "ultra_fine", true), // Export VTK
    ];
...
fn run_single_config(
    ...
    let load_bc = LoadCondition::new(x_max_nodes.clone(), DVector::from_vec(vec![0.0, 0.0, -p]));
    ...
    let mid_deflections: Vec<f64> = nodes
        .iter()
    ...
    simulation
        .node_fields
        .insert("displacement_magnitude".to_string(), disp_mag);
    simulation
        .node_fields
        .insert("von_mises_stress".to_string(), vm_stress);
    ...
    info!(
        "  Mesh: {}×{}×{}, Computed tip deflection: {:.6e} m, Analytical: {:.6e} m",
...
mod tests {
    ...
    fn test_cantilever_beam_benchmark() {
        ...
        for metric in &result.metrics {
            println!(
                "  {}: analytical={:.4e}, computed={:.4e}, error={:.2e}",

...
fn generate_comparison_table(c3d8: &[MeshResult], c3d20: &[MeshResult], analytical: f64) -> String {
    ...
    output.push_str(&format!(
        "Analytical tip deflection: {:.6e} m\n\n",
        analytical
    ));
    ...
    if let (Some(c3d8_fine), Some(c3d20_coarse)) = (c3d8.last(), c3d20.first()) {
        if c3d20_coarse.error_percent < c3d8_fine.error_percent {
            ...
            output.push_str(&format!(
                ...
                c3d20_coarse.dof_count,
                c3d20_coarse.error_percent,
                c3d8_fine.dof_count,
                c3d8_fine.error_percent,
                dof_ratio
            ));
        }
    }
...
fn generate_comparison_svg(c3d8: &[MeshResult], c3d20: &[MeshResult], analytical: f64) -> String {
    ...
    let all_err: Vec<f64> = all_results
        .iter()
        .map(|r| r.error_percent.max(0.1))
        .collect();

    let log_dof_min = all_dof
        .iter()
        .map(|x| x.log10())
        .fold(f64::INFINITY, f64::min);
    let log_dof_max = all_dof
        .iter()
        .map(|x| x.log10())
        .fold(f64::NEG_INFINITY, f64::max);
    let log_err_min = all_err
        .iter()
        .map(|x| x.log10())
        .fold(f64::INFINITY, f64::min);
    let log_err_max = all_err
        .iter()
        .map(|x| x.log10())
        .fold(f64::NEG_INFINITY, f64::max);
    ...
    let scale_x = |dof: f64| -> f64 {
        margin as f64
            + (dof.log10() - log_dof_min) / (log_dof_max - log_dof_min) * plot_width as f64
    };
    let scale_y = |err: f64| -> f64 {
        margin as f64
            + (log_err_max - err.max(0.1).log10()) / (log_err_max - log_err_min)
                * plot_height as f64
    };
...
fn generate_timing_svg(c3d8: &[MeshResult], c3d20: &[MeshResult]) -> String {
    ...
    // Get data ranges (log scale for both axes)
    let all_dof: Vec<f64> = all_results.iter().map(|r| r.dof_count as f64).collect();
    let all_time: Vec<f64> = all_results
        .iter()
        .map(|r| r.solve_time_ms.max(0.01))
        .collect();
    ...
    let log_dof_max = all_dof
        ...
        .map(|x| x.log10())
        .fold(f64::NEG_INFINITY, f64::max);
    let log_time_min = all_time
        .iter()
        .map(|x| x.log10())
        .fold(f64::INFINITY, f64::min);
    let log_time_max = all_time
        .iter()
        .map(|x| x.log10())
        .fold(f64::NEG_INFINITY, f64::max);
    ...
    let scale_y = |time: f64| -> f64 {
        margin as f64
            + (log_time_max - time.max(0.01).log10()) / (log_time_max - log_time_min)
                * plot_height as f64
    };
...
mod tests {
    ...
    fn test_c3d20_more_accurate() {
        ...
        println!(
            "  C3D8:  {} DOF, {:.2}% error",
            c3d8.dof_count, c3d8.error_percent
        );
        println!(
            "  C3D20: {} DOF, {:.2}% error",
            c3d20.dof_count, c3d20.error_percent
        );

...
pub fn run() -> BenchmarkResult {
    ...
    let u_inner =
        (1.0 / e) * ((1.0 - 2.0 * nu) * const_a * A + (1.0 + nu) * const_b / (2.0 * A.powi(2)));
    let u_outer =
        (1.0 / e) * ((1.0 - 2.0 * nu) * const_a * B + (1.0 + nu) * const_b / (2.0 * B.powi(2)));

    info!(
        "  σ_r at inner surface: {:.4e} Pa (should be {:.4e} Pa)",
        sigma_r_inner, -P_I
    );
    info!(
        "  σ_r at outer surface: {:.4e} Pa (should be {:.4e} Pa)",
        sigma_r_outer, -P_O
    );
    ...
    for (n_r, n_phi, n_theta, mesh_name) in refinements {
        info!(
            "Running with {} mesh ({}×{}×{} divisions)",
            mesh_name, n_r, n_phi, n_theta
        );
...
fn run_single_mesh(
    ...
    // Generate mesh
    let mesh =
        generate_hollow_sphere_sector_mesh(A, B, n_radial, n_phi, n_theta, phi_max, theta_max);
        ...
        if let Some(&(x, y, z)) = node_coords.get(&node_id) {
            ...
            if r > 1e-10 {
                ...
                if fix_x.is_some() || fix_y.is_some() || fix_z.is_some() {
                    let bc = FixedCondition::new(vec![node_id], vec![fix_x, fix_y, fix_z]);
                    simulation.add_boundary_condition(Box::new(bc));
                }
            }
        }
    ...
    let avg_u_r_error = if count > 0 {
        total_u_r_error / count as f64
    } else {
        0.0
    };
    ...
    let inner_disp = inner_nodes
        ...
        .filter_map(|node| {
            ...
            if r > 1e-10 {
                let u_r =
                    (node.displacement.x * x + node.displacement.y * y + node.displacement.z * z)
                        / r;
                Some(u_r)
            } else {
                None

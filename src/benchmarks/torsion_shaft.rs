...
pub fn run() -> BenchmarkResult {
    ...
    info!(
        "  Analytical twist angle (circular): {:.6} rad = {:.4}°",
        twist_angle,
        twist_angle.to_degrees()
    );
    info!(
        "  Analytical max shear stress (circular): {:.2e} Pa",
        tau_max
    );
    ...
    for (nx, ny, nz, mesh_name, export_vtk) in refinements {
        info!(
            "Running with {} mesh ({} x {} x {} elements)",
            mesh_name, nx, ny, nz
        );
...
fn run_single_mesh(
    ...
    let corner_disp: Vec<f64> = x_max_nodes
        .iter()
    ...
    let max_axial_disp = x_max_nodes
        .iter()
        .filter_map(|&node_id| nodes.get(node_id))
        .map(|node| node.displacement.x.abs())
        .fold(0.0_f64, f64::max);

    let max_tangential_disp = x_max_nodes
        .iter()
        .filter_map(|&node_id| nodes.get(node_id))
        .map(|node| {
            let u_y = node.displacement.y;
    ...
    let computed_twist = if !corner_disp.is_empty() {
        corner_disp
            .iter()
            .filter(|&&x| x.is_finite() && x.abs() > 1e-15)
            .sum::<f64>()
            / corner_disp
                .iter()
                .filter(|&&x| x.is_finite() && x.abs() > 1e-15)
    ...
    info!(
        "  Computed twist angle: {:.6} rad = {:.4}°",
        computed_twist,
        computed_twist.to_degrees()
    );
    info!(
        "  Analytical twist (square): {:.6} rad = {:.4}°",
        analytical_twist_square,
        analytical_twist_square.to_degrees()
    );

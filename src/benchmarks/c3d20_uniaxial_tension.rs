...
pub fn run() -> BenchmarkResult {
    ...
    for (nx, ny, nz, mesh_name) in refinements {
        info!("Running C3D20 uniaxial tension benchmark with {} mesh ({} x {} x {} elements)", 
              mesh_name, nx, ny, nz);
...
mod tests {
    ...
    fn test_c3d20_uniaxial_tension_benchmark() {
        let result = run();
        assert!(result.passed, "C3D20 uniaxial tension benchmark failed: {:?}", result.metrics);
    }
    ...
    fn test_c3d20_mesh_generation() {
        ...
        assert_eq!(mesh.nodes.len(), 20, "Expected 20 nodes for 1x1x1 C3D20 mesh");
        assert_eq!(mesh.elements.len(), 1, "Expected 1 element for 1x1x1 C3D20 mesh");
        
        // Check that element has 20 nodes
        let elem = mesh.elements.get(&0).unwrap();
        assert_eq!(elem.connectivity.len(), 20, "C3D20 element should have 20 nodes");
        ...
        for &node_id in &x_min {
            assert!(elem.connectivity.contains(&node_id), 
                "x_min node {} not in element connectivity", node_id);
        }
        for &node_id in &x_max {
            assert!(elem.connectivity.contains(&node_id),
                "x_max node {} not in element connectivity", node_id);
        }
    }
}

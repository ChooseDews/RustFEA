...
fn generate_test_system(n_nodes: usize) -> (usize, Vec<(usize, usize, f64)>, Vec<f64>) {
    ...
    let rhs: Vec<f64> = (0..n_dofs).map(|i| (i as f64 * 0.01).sin() * 100.0).collect();
    (n_dofs, triplets, rhs)
}
...
pub fn run_solver_comparison() -> Vec<SolverBenchmarkResult> {
    ...
    for n_nodes in node_counts {
        ...
        println!("Testing system: {} nodes, {} DOFs, {} nonzeros", n_nodes, n_dofs, n_nonzeros);
        ...
        let diff_norm: f64 = faer_solution.iter()
        ...
        println!("  nalgebra: {:7.2} ms, {:+6.1} MB", nalgebra_time, nalgebra_mem);
        println!("  rsparse:  {:7.2} ms, {:+6.1} MB", rsparse_time, rsparse_mem);
        println!("  speedup: faer {:.2}x, rsparse {:.2}x vs nalgebra\n", faer_vs_nalgebra, rsparse_vs_nalgebra);
    ...
    for r in &results {
        println!("{:>8} {:>11.2}ms {:>11.2}ms {:>11.2}ms {:>11.2}x {:>13.2}x", 
            r.n_dofs, r.faer_time_ms, r.nalgebra_time_ms, r.rsparse_time_ms,
            r.faer_vs_nalgebra, r.rsparse_vs_nalgebra);
    }
    ...
    for r in &results {
        println!("{:>8} {:>+11.1}MB {:>+11.1}MB {:>+11.1}MB", 
            r.n_dofs, r.faer_mem_mb, r.nalgebra_mem_mb, r.rsparse_mem_mb);
    }
    
    // Averages
    let avg_faer_speed: f64 = results.iter().map(|r| r.faer_vs_nalgebra).sum::<f64>() / results.len() as f64;
    let avg_rsparse_speed: f64 = results.iter().map(|r| r.rsparse_vs_nalgebra).sum::<f64>() / results.len() as f64;
    ...
    if avg_rsparse_speed > 1.0 {
        println!("  ✅ rsparse is {:.0}% FASTER", (avg_rsparse_speed - 1.0) * 100.0);
    } else {
        println!("  ❌ rsparse is {:.0}% slower", (1.0 - avg_rsparse_speed) * 100.0);
    }

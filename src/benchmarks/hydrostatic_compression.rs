...
pub fn run() -> BenchmarkResult {
    ...
    info!(
        "Running hydrostatic compression benchmark with {} x {} x {} mesh",
        nx, ny, nz
    );
...
fn run_single_mesh(nx: usize, ny: usize, nz: usize, e: f64, nu: f64) -> BenchmarkResult {
    ...
    let corner_nodes: Vec<usize> = mesh
        .nodes
        .iter()
        .filter(|(_, node)| {
    ...
    let boundary_node_coords: std::collections::HashMap<usize, (f64, f64, f64)> = boundary_nodes
        .iter()
        .filter_map(|&id| {
            mesh.nodes
                .get(&id)
                .map(|n| (id, (n.coordinates[0], n.coordinates[1], n.coordinates[2])))
        })
        .collect();
    ...
    if !corner_nodes.is_empty() {
        let bc = FixedCondition::new(corner_nodes.clone(), vec![Some(0.0), Some(0.0), Some(0.0)]);
    ...
    for (&node_id, &(x, y, z)) in boundary_node_coords.iter() {
        ...
        let bc = FixedCondition::new(vec![node_id], vec![Some(u_x), Some(u_y), Some(u_z)]);
        simulation.add_boundary_condition(Box::new(bc));
    }
...
mod tests {
    ...
    fn test_hydrostatic_compression_benchmark() {
        ...
        assert!(
            result.passed,
            "Hydrostatic compression benchmark failed: {:?}",
            result.metrics
        );
    }
}

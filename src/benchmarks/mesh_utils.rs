...
use crate::mesh::{Body, ElementGroup, MeshAssembly, MeshElement, MeshNode, NodeGroup};
...
pub fn generate_block_mesh(
    ...
    for k in 0..=n_z {
        for j in 0..=n_y {
            for i in 0..=n_x {
                ...
                // Categorize by face
                if i == 0 {
                    x_min_nodes.push(node_id);
                }
                if i == n_x {
                    x_max_nodes.push(node_id);
                }
                if j == 0 {
                    y_min_nodes.push(node_id);
                }
                if j == n_y {
                    y_max_nodes.push(node_id);
                }
                if k == 0 {
                    z_min_nodes.push(node_id);
                }
                if k == n_z {
                    z_max_nodes.push(node_id);
                }
    ...
    node_groups.insert(
        "x_min".to_string(),
        NodeGroup {
            nodes: x_min_nodes.clone(),
            name: "x_min".to_string(),
        },
    );
    node_groups.insert(
        "x_max".to_string(),
        NodeGroup {
            nodes: x_max_nodes.clone(),
    ...
    node_groups.insert(
        "y_min".to_string(),
        NodeGroup {
            nodes: y_min_nodes.clone(),
            name: "y_min".to_string(),
        },
    );
    node_groups.insert(
        "y_max".to_string(),
        NodeGroup {
            nodes: y_max_nodes.clone(),
            name: "y_max".to_string(),
        },
    );
    node_groups.insert(
        "z_min".to_string(),
        NodeGroup {
            nodes: z_min_nodes.clone(),
    ...
    node_groups.insert(
        "z_max".to_string(),
        NodeGroup {
            nodes: z_max_nodes.clone(),
            name: "z_max".to_string(),
        },
    );

    // Create element group
    element_groups.insert(
...
pub fn generate_block_mesh_c3d20(
    ...
    let grid_index = |i: usize, j: usize, k: usize| -> usize { i + j * nn_x + k * nn_x * nn_y };
...
pub fn generate_hollow_sphere_sector_mesh(
    ...
    for i in 0..=n_radial {
        ...
        for j in 0..=n_phi {
            ...
            for k in 0..=n_theta {
                ...
                if i == 0 {
                    inner_nodes.push(node_id);
                }
                if i == n_radial {
                    outer_nodes.push(node_id);
                }
                if k == 0 {
                    x_sym_nodes.push(node_id);
                } // x-z plane
                if k == n_theta {
                    y_sym_nodes.push(node_id);
                } // y-z plane (if theta_max = PI/2)
                if j == n_phi {
                    z_sym_nodes.push(node_id);
                } // x-y plane (if phi_max = PI/2)
    ...
    node_groups.insert(
        "x_sym".to_string(),
        NodeGroup {
            nodes: x_sym_nodes.clone(),
            name: "x_sym".to_string(),
        },
    );
    node_groups.insert(
        "y_sym".to_string(),
        NodeGroup {
            nodes: y_sym_nodes.clone(),
            name: "y_sym".to_string(),
        },
    );
    node_groups.insert(
        "z_sym".to_string(),
        NodeGroup {
            nodes: z_sym_nodes.clone(),
            name: "z_sym".to_string(),
        },
    );

    // Create element group
    element_groups.insert(
        "sphere_body".to_string(),
        ElementGroup {
            elements: all_elements.clone(),
            name: "sphere_body".to_string(),
            el_type: "C3D8".to_string(),

...
pub fn run() -> BenchmarkResult {
    ...
    for (nx, ny, nz, mesh_name, export_vtk) in mesh_configs {
        ...
        let mesh_result =
            run_prescribed_approach(R, delta, a, nx, ny, nz, e, nu, export_vtk, mesh_name);
...
fn run_prescribed_approach(
    ...
    let mut mesh = generate_block_mesh(
        domain_size,
        domain_size,
        domain_size / 2.0,
        n_x,
    ...
    let top_nodes: Vec<_> = z_max_nodes
        .iter()
        .filter_map(|&id| {
    ...
    let center_disp: Vec<f64> = top_nodes
        .iter()
        .filter(|(x, y, _)| {
            x.abs() < domain_size / n_x as f64 && y.abs() < domain_size / n_y as f64
        })
        .map(|(_, _, uz)| *uz)
        .collect();
    ...
    let edge_disp: Vec<f64> = top_nodes
        .iter()
        .filter(|(x, y, _)| {
            let r = (x * x + y * y).sqrt();

...
pub fn run() -> BenchmarkResult {
    ...
    for (nx, ny, nz, mesh_name) in refinements {
        info!("Running pure shear benchmark with {} mesh ({} x {} x {} elements)", 
              mesh_name, nx, ny, nz);
...
fn run_single_mesh(nx: usize, ny: usize, nz: usize) -> BenchmarkResult {
    ...
    let boundary_node_coords: std::collections::HashMap<usize, f64> = all_boundary_nodes.iter()
    ...
    for (&node_id, &y) in boundary_node_coords.iter() {
        ...
        let bc = FixedCondition::new(
            vec![node_id],
            vec![Some(u_x), Some(0.0), Some(0.0)],
        );
    ...
    for node in nodes.iter() {
        ...
        if u_x_err > max_u_x_error { max_u_x_error = u_x_err; }
        if u_y_err > max_u_y_error { max_u_y_error = u_y_err; }
        if u_z_err > max_u_z_error { max_u_z_error = u_z_err; }
    }
...
mod tests {
    ...
    fn test_pure_shear_benchmark() {
        let result = run();
        assert!(result.passed, "Pure shear benchmark failed: {:?}", result.metrics);
    }
}
